...
public class CustomReloadableResourceBundleMessageSource extends AbstractMessageSource implements ResourceLoaderAware {
    ...
    /**
     * Set whether to fall back to the system Locale if no files for a specific Locale have been found. Default is "true"; if this
     * is turned off, the only fallback will be the default file (e.g. "messages.properties" for basename "messages").
     * <p>
     * Falling back to the system Locale is the default behavior of {@code java.util.ResourceBundle}. However, this is often not
     ...
     * <ul>
     * <li>Default is "-1", indicating to cache forever (just like {@code java.util.ResourceBundle}).
     * <li>A positive number will cache loaded properties files for the given number of seconds. This is essentially the interval
     * between refresh checks. Note that a refresh attempt will first check the last-modified timestamp of the file before
     * actually reloading it; so if files don't change, this interval can be set rather low, as refresh attempts will not actually
     * reload.
     * <li>A value of "0" will check the last-modified timestamp of the file on every message access. <b>Do not use this in a
     ...
     * bundles. Either fetches the holder from the cache or freshly loads it.
     * <p>
     * Only used when caching resource bundle contents forever, i.e. with cacheSeconds < 0. Therefore, merged properties are
    ...
    protected PropertiesHolder getProperties(String filename) {
        synchronized (this.cachedProperties) {
            ...
            if (propHolder != null
                    && (propHolder.getRefreshTimestamp() < 0 || propHolder.getRefreshTimestamp() > System.currentTimeMillis()
                            - this.cacheMillis)) {
                // up to date
                return propHolder;
            }
    ...
    public void clearCacheIncludingAncestors() {
        clearCache();
        if (getParentMessageSource() instanceof CustomReloadableResourceBundleMessageSource) {
            ((CustomReloadableResourceBundleMessageSource) getParentMessageSource()).clearCacheIncludingAncestors();
        }
    }
    ...
    /**
     * PropertiesHolder for caching. Stores the last-modified timestamp of the source file for efficient change detection, and the
     * timestamp of the last refresh attempt (updated every time the cache entry gets re-validated).
     */
